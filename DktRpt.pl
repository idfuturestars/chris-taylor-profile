...
<li class="yuimenubaritem"><a class="yuimenubaritemlabel" href='/cgi-bin/login.pl?logout'>Log Out</a></li></ul><hr class="hrmenuseparator"></div></div></div><script type="text/javascript">if (navigator.appVersion.indexOf("MSIE")==-1){window.setTimeout(CMECF.MainMenu.createMenu, 0);}else{CMECF.util.Event.addListener(window, "load", CMECF.MainMenu.createMenu);}</script>  <div id="cmecfMainContent"><input type="hidden" id="cmecfMainContentScroll" value="0">
		<br>
		<div><b>Warning:</b> The link to this page may not have originated from within CM/ECF.</div>
		<div style='margin-left: 4em'>If you click the Continue link, you will be billed for the information provided.</div>
		<form action='/cgi-bin/DktRpt.pl?394405483699449-L_9999_2-1' method='POST' id='referrer_form'>
			<input type='hidden' name='csrf' value='0.259115669268311'>
			<p><a href="javascript:void(0);" onClick="document.getElementById('referrer_form').submit();">Continue</a></p>
		</form>
		<!-- Note to data miners: set your REFERER header to https://external to avoid this message.  This was added to prevent a CSRF. -->
